...
// bus. They are not symmetric, and the asymmetry is a departure from the
// specification: DDC/CI puts both addresses into a get's checksum, and a get
// seeded that way is not what displays on Apple silicon answer. Seeding a get
// with the host address alone is what is proven against them, so it is what
// this does.
