...
//
//  The constants come from Apple's own headers: the key codes from
//  `ev_keymap.h` and the subtype from `IOLLEvent.h`. How the key code and its
//  state are packed into `data1` is not in either of them, so that part is
//  pinned by events captured off a real keyboard rather than by reasoning —
//  see `CapturedMediaKeyTests`.
//
