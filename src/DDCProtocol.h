...
//
//  Where this departs from the DDC/CI specification the departure is marked,
//  and the reason is the same every time: what the specification describes and
//  what displays on Apple silicon actually answer are not the same thing, and
//  the captures decide. Checks made beyond what the specification asks for are
//  marked too.
//
...
///
/// Four checks, where the obvious reading makes one. The checksum alone is not
...
/// quiet setting. Muting is a different thing from being quiet, and it is the
/// mute control's job, so the floor is one step rather than 0: landing a step
/// louder than asked is the smaller error.
